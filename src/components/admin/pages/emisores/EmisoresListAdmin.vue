...
const rutaAPI = import.meta.env.VITE_APP_RUTA_API;
  ...
  computed: {
     EmisoresFiltrados() {
      return this.emisores.filter((estatu) => {
        const termino = this.filtro.toLowerCase();
        return (
            estatu.nombre_emisor.toLowerCase().includes(termino) 
  ...
  async mounted() {
    ...
    try {
      const response = await axios.get(`${rutaAPI}/api/emisores`);
  ...
  methods: {
    editar(id) {
      ...
      this.$router.push(`/admin/emisores/actualizar/${id}`);
    },
  },
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header card-header text-white bg-primary">
                <h3 class="card-title">Listado de Emisores Registrados </h3>

                <div class="card-tools">
                  <div class="input-group input-group-sm" style="width: 150px;">
                    <input type="text"  v-model="filtro"   name="table_search" class="form-control float-right"  placeholder="Filtrar por Nombre emisores">
              ...
              <div class="card-body table-responsive p-0">
                <table class="table table-hover text-nowrap">
                  ...
                  <tbody>
                  <tr v-for="estatu in EmisoresFiltrados" :key="estatu.id">
                   
                    <td>{{ estatu.nombre_emisor }}</td>
